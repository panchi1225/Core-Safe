...
const DisasterCouncilWizard: React.FC<Props> = ({ initialData, initialDraftId, onBackToMenu }) => {
  ...
  const renderStep1 = () => (
    <div className="space-y-6">
      ...
      <div className="form-control"><label className="label font-bold text-gray-700">施工者名 (マスタ選択)</label><select className="w-full p-3 border border-gray-300 rounded-lg bg-white text-black outline-none appearance-none" value={report.contractor} onChange={(e) => updateReport('contractor', e.target.value)}>{masterData.contractors.map(c => <option key={c} value={c}>{c}</option>)}</select></div>
    </div>
  );
  ...
  const renderStep2 = () => (
    <div className="space-y-8">
      ...
      <div className="bg-gray-50 p-4 rounded border"><h3 className="font-bold text-gray-700 mb-3">開催情報</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label className="text-xs font-bold text-gray-500">場所 (マスタ選択)</label><select className="w-full p-2 border border-gray-300 rounded bg-white text-black outline-none appearance-none" value={report.location} onChange={(e) => updateReport('location', e.target.value)}>{masterData.locations.map(s => <option key={s} value={s}>{s}</option>)}</select></div><div><label className="text-xs font-bold text-gray-500">時間</label><div className="flex items-center gap-2"><input type="time" className="p-2 border rounded" value={report.startTime} onChange={(e) => updateReport('startTime', e.target.value)} /><span>～</span><input type="time" className="p-2 border rounded" value={report.endTime} onChange={(e) => updateReport('endTime', e.target.value)} /></div></div></div></div>
      <div className="bg-white p-4 rounded border shadow-sm"><h3 className="font-bold text-gray-700 mb-3 border-b pb-1">元請出席者</h3><div className="grid grid-cols-1 md:grid-cols-2 gap-4">{report.gcAttendees.map((attendee, idx) => (<div key={idx} className="flex gap-2 items-center"><input type="text" placeholder="職務名" className="w-1/3 p-2 border border-gray-300 rounded text-sm bg-gray-50" value={attendee.role} onChange={(e) => updateGCAttendee(idx, 'role', e.target.value)} /><select className="flex-1 p-2 border border-gray-300 rounded text-sm appearance-none" value={attendee.name} onChange={(e) => updateGCAttendee(idx, 'name', e.target.value)}><option value="">(選択してください)</option>{masterData.supervisors.map(s => <option key={s} value={s}>{s}</option>)}</select></div>))}</div></div>
