        ...
        <div className="space-y-6 max-w-3xl mx-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <div className="flex gap-1 items-center">
                  ...
                  <select className="w-16 p-2 border rounded text-center appearance-none bg-white" value={data.birthYear} onChange={e => handleChange('birthYear', parseInt(e.target.value))}>
                    <option value="">-</option>
                    {range(1, data.birthEra === 'Showa' ? 64 : 31).map(y => <option key={y} value={y}>{y}</option>)}
                  </select>
                  <span>年</span>
                  <select className="w-14 p-2 border rounded text-center appearance-none bg-white" value={data.birthMonth} onChange={e => handleChange('birthMonth', parseInt(e.target.value))}>
                    <option value="">-</option>
                    {range(1, 12).map(m => <option key={m} value={m}>{m}</option>)}
                  </select>
                  <span>月</span>
                  <select className="w-14 p-2 border rounded text-center appearance-none bg-white" value={data.birthDay} onChange={e => handleChange('birthDay', parseInt(e.target.value))}>
                    <option value="">-</option>
              ...
              <div>
                ...
                <div className="flex gap-2 items-center">
                  <select className="w-20 p-2 border rounded text-center appearance-none bg-white" value={data.experienceYears} onChange={e => handleChange('experienceYears', parseInt(e.target.value))}>
                    ...
                    {range(1, 60).map(y => <option key={y} value={y}>{y}</option>)}
                  </select>
                  <span>年</span>
                  <select className="w-16 p-2 border rounded text-center appearance-none bg-white" value={data.experienceMonths} onChange={e => handleChange('experienceMonths', parseInt(e.target.value))}>
                    <option value="0">0</option>
                    {range(1, 11).map(m => <option key={m} value={m}>{m}</option>)}
                  </select>
                  <span>ヶ月</span>
                </div>
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-red-50 p-4 rounded border border-red-100">
              ...
              <div>
                ...
                <select className="w-full p-2 border rounded bg-white" value={data.emergencyContactRelation} onChange={e => handleChange('emergencyContactRelation', e.target.value)}>
                  <option value="">選択してください</option>
          ...
          <section>
            ...
            <div className="flex gap-1 items-center bg-green-50 p-4 rounded border border-green-100">
              ...
              <select className="w-16 p-2 border rounded text-center appearance-none bg-white" value={data.healthCheckYear} onChange={e => handleChange('healthCheckYear', parseInt(e.target.value))}>
                <option value="">-</option>
                {range(1, 30).map(y => <option key={y} value={y}>{y}</option>)}
              </select>
              <span>年</span>
              <select className="w-14 p-2 border rounded text-center appearance-none bg-white" value={data.healthCheckMonth} onChange={e => handleChange('healthCheckMonth', parseInt(e.target.value))}>
                <option value="">-</option>
                {range(1, 12).map(m => <option key={m} value={m}>{m}</option>)}
              </select>
              <span>月</span>
              <select className="w-14 p-2 border rounded text-center appearance-none bg-white" value={data.healthCheckDay} onChange={e => handleChange('healthCheckDay', parseInt(e.target.value))}>
                <option value="">-</option>
                {range(1, 31).map(d => <option key={d} value={d}>{d}</option>)}
              </select>
              <span>日</span>
