...
const LABEL_MAP: Record<string, string> = {
  ...
  jobTypes: '職種',
  roles: '役職',
  workplaces: '作業所名', // NewcomerSurvey独自
};
...
// -----------------------------------------------------------------------------
// 内部コンポーネント: マスタ管理セクション (タブ + リスト選択式)
// -----------------------------------------------------------------------------
        ...
        {!selectedKey ? (
          ...
          <div className="animate-fade-in">
            <div className="flex items-center gap-2 mb-4">
              <button onClick={() => setSelectedKey(null)} className="text-sm text-blue-600 hover:underline">
                &lt; 戻る
              </button>
                ...
                <ul className="divide-y">
                  {((masterData as any)[selectedKey] || []).map((item: string, idx: number) => (
                    <li key={idx} className="p-2 flex justify-between items-center hover:bg-gray-50">
                      <span>{item}</span>
                      <button
                        onClick={() => handleDeleteRequest(idx, item)}
...
const NewcomerSurveyWizard: React.FC<Props> = ({ initialData, initialDraftId, onBackToMenu }) => {
  ...
  const sanitizeReportData = (data?: any): NewcomerSurveyReportData => {
    ...
    if (data.name && (!data.nameSei || !data.nameMei)) {
      ...
      if (parts.length >= 2) {
        merged.nameSei = parts[0];
        merged.nameMei = parts.slice(1).join(' ');
      } else {
        merged.nameSei = data.name;
        merged.nameMei = '';
      }
    }
    ...
    if (data.furigana && (!data.furiganaSei || !data.furiganaMei)) {
      ...
      if (parts.length >= 2) {
        merged.furiganaSei = parts[0];
        merged.furiganaMei = parts.slice(1).join(' ');
      } else {
        merged.furiganaSei = data.furigana;
        merged.furiganaMei = '';
      }
    }
    ...
    if (data.emergencyContactName && (!data.emergencyContactNameSei || !data.emergencyContactNameMei)) {
      ...
      if (parts.length >= 2) {
        merged.emergencyContactNameSei = parts[0];
        merged.emergencyContactNameMei = parts.slice(1).join(' ');
      } else {
        merged.emergencyContactNameSei = data.emergencyContactName;
        merged.emergencyContactNameMei = '';
      }
    }
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      if (width < 640) setPreviewScale(0.45);
      else if (width < 1024) setPreviewScale(0.6);
  ...
  const validateStep1 = () => {
    ...
    if (!report.project) newErrors.project = true;
    if (!report.director) newErrors.director = true;
    if (!report.company) newErrors.company = true;
    if (!report.workplace) newErrors.workplace = true;
    if (!report.nameSei) newErrors.nameSei = true;
    if (!report.nameMei) newErrors.nameMei = true;
    if (!report.furiganaSei) newErrors.furiganaSei = true;
    if (!report.furiganaMei) newErrors.furiganaMei = true;
    if (!report.birthDate) newErrors.birthDate = true;
    if (!report.bloodType) newErrors.bloodType = true;
    if (!report.address) newErrors.address = true;
    if (!report.phone) newErrors.phone = true;
    if (!report.emergencyContactNameSei) newErrors.emergencyContactNameSei = true;
    if (!report.emergencyContactNameMei) newErrors.emergencyContactNameMei = true;
    if (!report.emergencyContactRelation) newErrors.emergencyContactRelation = true;
    if (!report.emergencyContactPhone) newErrors.emergencyContactPhone = true;
    if (!report.role) newErrors.role = true;
    if (!report.jobType) newErrors.jobType = true;
    if (!report.experienceYears) newErrors.experienceYears = true;
    if (!report.healthCheckDate) newErrors.healthCheckDate = true;
    if (!report.bloodPressureHigh) newErrors.bloodPressureHigh = true;
    if (!report.bloodPressureLow) newErrors.bloodPressureLow = true;
    ...
    <div className="min-h-screen bg-gray-100 pb-20 font-sans text-gray-800">
      ...
      <div className="bg-white shadow-sm sticky top-0 z-40 px-4 py-3 flex justify-between items-center">
        <div className="flex items-center gap-3">
          <button onClick={handleHomeClick} className="text-gray-500 hover:text-gray-700">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => setShowPreview(true)} className="px-3 py-1.5 bg-blue-100 text-blue-700 rounded text-sm hover:bg-blue-200">
            <i className="fa-solid fa-eye mr-1"></i>プレビュー
          </button>
          <button onClick={handleTempSave} disabled={isSaving} className="px-3 py-1.5 bg-green-600 text-white rounded text-sm hover:bg-green-700 disabled:opacity-50">
            <i className="fa-solid fa-floppy-disk mr-1"></i>一時保存
          </button>
        </div>
      </div>
      ...
      <div className="max-w-3xl mx-auto p-4">
        {/* Master Manager */}
        {showMasterManager && (
          <MasterSection masterData={masterData} onUpdate={handleMasterUpdate} />
        )}
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          {step === 1 && (
            <div className="animate-fade-in space-y-6">
              <h3 className="text-xl font-bold border-b pb-2 mb-4">基本情報入力</h3>
              <p className="text-xs text-red-500 mb-2">※すべての項目が必須です</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select 
                    ...
                    onChange={e => updateReport({ workplace: e.target.value })} 
              ...
              <div className="border-t pt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2 flex gap-4">
                  <div className="flex-1">
                    <label className="block text-sm font-bold text-gray-700 mb-1">氏名 (姓) <span className="text-red-500 text-xs">※必須</span></label>
                    <input 
                      type="text" 
                      value={report.nameSei || ''} 
                      onChange={e => updateReport({ nameSei: e.target.value })} 
                      className={`w-full h-11 border rounded px-3 ${getErrorClass('nameSei')}`}
                      placeholder="例: 山田"
                    />
                  </div>
                  <div className="flex-1">
                    <label className="block text-sm font-bold text-gray-700 mb-1">氏名 (名) <span className="text-red-500 text-xs">※必須</span></label>
                    <input 
                      type="text" 
                      value={report.nameMei || ''} 
                      onChange={e => updateReport({ nameMei: e.target.value })} 
                      className={`w-full h-11 border rounded px-3 ${getErrorClass('nameMei')}`}
                      placeholder="例: 太郎"
                    />
                  </div>
                </div>
                ...
                <div className="md:col-span-2 flex gap-4">
                  <div className="flex-1">
                    <label className="block text-sm font-bold text-gray-700 mb-1">フリガナ (セイ) <span className="text-red-500 text-xs">※必須</span></label>
                    <input 
                      type="text" 
                      value={report.furiganaSei || ''} 
                      onChange={e => updateReport({ furiganaSei: e.target.value })} 
                      className={`w-full h-11 border rounded px-3 ${getErrorClass('furiganaSei')}`}
                      placeholder="例: ヤマダ"
                    />
                  </div>
                  <div className="flex-1">
                    <label className="block text-sm font-bold text-gray-700 mb-1">フリガナ (メイ) <span className="text-red-500 text-xs">※必須</span></label>
                    <input 
                      type="text" 
                      value={report.furiganaMei || ''} 
                      onChange={e => updateReport({ furiganaMei: e.target.value })} 
                      className={`w-full h-11 border rounded px-3 ${getErrorClass('furiganaMei')}`}
                      placeholder="例: タロウ"
                    />
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">生年月日 <span className="text-red-500 text-xs">※必須</span></label>
                  <input 
                    type="date" 
                    value={report.birthDate} 
                    onChange={e => updateReport({ birthDate: e.target.value })} 
                    className={`w-full h-11 border rounded px-3 ${getErrorClass('birthDate')}`} 
                  />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-700 mb-1">血液型 <span className="text-red-500 text-xs">※必須</span></label>
                  <div className="flex gap-2">
                    <select 
                      value={report.bloodType} 
                      onChange={e => updateReport({ bloodType: e.target.value })} 
                    ...
                    <select 
                      value={report.bloodTypeRh} 
                      onChange={e => updateReport({ bloodTypeRh: e.target.value as any })} 
              ...
              <div className="border-t pt-4 space-y-4">
                ...
                <div>
                  ...
                  <input 
                    type="tel" 
                    value={report.phone} 
                    onChange={e => updateReport({ phone: e.target.value })} 
                    className={`w-1/2 h-11 border rounded px-3 ${getErrorClass('phone')}`} 
              ...
              {/* 緊急連絡先エリア - デザイン調整 */}
              <div className="border-t pt-4 bg-red-50 p-4 rounded border border-red-100">
                <h4 className="font-bold text-red-800 mb-2 text-sm">緊急連絡先 <span className="text-red-500 text-xs">※必須</span></h4>
                <div className="grid grid-cols-1 gap-3">
                   ...
                   <div className="flex gap-3 max-w-lg">
                    <div className="flex-1">
                      ...
                      <input 
                        type="text" 
                        value={report.emergencyContactNameSei || ''} 
                        onChange={e => updateReport({ emergencyContactNameSei: e.target.value })} 
                    ...
                    <div className="flex-1">
                      ...
                      <input 
                        type="text" 
                        value={report.emergencyContactNameMei || ''} 
                        onChange={e => updateReport({ emergencyContactNameMei: e.target.value })} 
                  ...
                  <div className="grid grid-cols-2 gap-3 max-w-lg">
                    <div>
                      ...
                      <input 
                        ...
                        className={`w-full h-11 border rounded px-3 ${getErrorClass('emergencyContactRelation')}`}
                        placeholder="例: 父"
                      />
                    </div>
                    <div>
              ...
              <div className="border-t pt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input 
                    ...
                    onChange={e => updateReport({ experienceYears: e.target.value })} 
              ...
              <div>
                ...
                <div className={`grid grid-cols-2 sm:grid-cols-4 gap-2 border p-2 rounded ${errors.jobType ? 'border-red-500 bg-red-50' : 'border-gray-200'}`}>
                  {JOB_TYPES.map(job => (
                ...
                {report.jobType === 'その他' && (
                  <input 
                    ...
                    placeholder="職種を入力" 
                    className="mt-2 w-full h-11 border rounded px-3" 
                  />
                )}
              </div>

              <div className="border-t pt-4">
                <label className="block text-sm font-bold text-gray-700 mb-2">健康診断 <span className="text-red-500 text-xs">※必須</span></label>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-blue-50 p-4 rounded border border-blue-100">
                   <div>
                     <label className="block text-xs text-gray-600 mb-1">受診年月</label>
                     <input 
                       type="month" 
                       value={report.healthCheckDate} 
                       onChange={e => updateReport({ healthCheckDate: e.target.value })} 
                       className={`w-full h-11 border rounded px-3 bg-white ${getErrorClass('healthCheckDate')}`} 
                     />
                   </div>
                   <div>
                     <label className="block text-xs text-gray-600 mb-1">血圧 (上)</label>
                     <input 
                       type="number" 
                       value={report.bloodPressureHigh} 
                       onChange={e => updateReport({ bloodPressureHigh: e.target.value })} 
                       className={`w-full h-11 border rounded px-3 ${getErrorClass('bloodPressureHigh')}`} 
                     />
                   </div>
                   <div>
                     <label className="block text-xs text-gray-600 mb-1">血圧 (下)</label>
                     <input 
                       type="number" 
                       value={report.bloodPressureLow} 
                       onChange={e => updateReport({ bloodPressureLow: e.target.value })} 
                       className={`w-full h-11 border rounded px-3 ${getErrorClass('bloodPressureLow')}`} 
                     />
                   </div>
                </div>
              </div>
                  ...
                  <div className="space-y-2">
                    {[
                      ...
                      <label key={q.k} className="flex items-center space-x-2">
                        <input 
                          type="checkbox" 
                          checked={(report.qualifications as any)?.[q.k] || false} 
                ...
                <div className="bg-gray-50 p-3 rounded flex flex-col">
                  ...
                  <div className="space-y-2 mb-4">
                    <label className="flex items-center space-x-2">
                      <input 
                        type="checkbox" 
                        checked={report.qualifications?.foreman || false} 
              ...
              <div className="text-center">
                ...
                {report.signatureDataUrl ? (
                  <div className="inline-block relative group">
                    ...
                    <button 
                      onClick={() => updateReport({ signatureDataUrl: undefined })}
                      className="absolute top-0 right-0 bg-red-500 text-white w-6 h-6 rounded-full flex items-center justify-center -mt-2 -mr-2 shadow-md hover:bg-red-600"
                    >
                  ...
                  <button 
                    ...
                    className="w-full max-w-sm py-8 border-2 border-dashed border-gray-300 rounded-lg text-gray-500 hover:bg-gray-50 hover:border-blue-500 hover:text-blue-500 transition flex flex-col items-center justify-center gap-2"
                  >
        ...
        <div className="mt-6 flex justify-between">
          <button 
            onClick={handleBack} 
            disabled={step === 1}
            className="px-6 py-3 rounded-lg bg-gray-200 text-gray-700 disabled:opacity-50 hover:bg-gray-300 transition font-bold"
          >
            戻る
          </button>
          ...
          {step < 3 ? (
            <button 
              onClick={handleNext}
              className="px-8 py-3 rounded-lg bg-blue-600 text-white hover:bg-blue-700 shadow-lg transition font-bold flex items-center gap-2"
            >
              次へ <i className="fa-solid fa-arrow-right"></i>
            </button>
          ) : (
            <button 
              onClick={handleSaveAndPrint}
              disabled={isSaving}
              className="px-8 py-3 rounded-lg bg-green-600 text-white hover:bg-green-700 shadow-lg transition font-bold flex items-center gap-2 disabled:opacity-70"
            >
              {isSaving ? (
                <><div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div> 保存中...</>
              ) : (
                <><i className="fa-solid fa-print"></i> 保存して印刷</>
              )}
            </button>
          )}
        </div>
      ...
      {showSigModal && (
        <div className="fixed inset-0 z-50 bg-gray-900 bg-opacity-90 flex flex-col">
          <div className="flex justify-between items-center p-4 text-white">
            ...
            <button onClick={() => setShowSigModal(false)} className="text-gray-400 hover:text-white">
          ...
          <div className="flex-1 bg-white p-4 flex items-center justify-center overflow-hidden relative">
             <SignatureCanvas 
               ref={sigCanvas} 
               penColor="black" 
               canvasProps={{ className: 'border-2 border-gray-300 rounded w-full h-full' }} 
             />
