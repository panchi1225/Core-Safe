...
const sanitizeReportData = (data: any): NewcomerSurveyReportData => {
  ...
  if (data) {
    ...
    // 新規作成時初期化
    // 保存エラー回避のため undefined ではなく null を使用
    base = {
      ...base,
      ...
      // ★修正: 誓約日が未設定の場合は、当日の日付(令和)を自動セット
...
const NewcomerSurveyWizard: React.FC<Props> = ({ initialData, initialDraftId, onBackToMenu }) => {
  ...
  const renderStep3 = () => (
    <div className="space-y-6">
      ...
      {/* ★修正: タップすると即座にモーダル(showSigModal)を表示 */}
  ...
  return (
    <>
      ...
      {/* 署名モーダル (showSigModalで制御、中間のポップアップなし) */}
      {showSigModal && (
