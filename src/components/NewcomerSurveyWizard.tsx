  ...
  return (
    <div className="border border-gray-200 p-4 rounded-lg bg-white shadow-sm break-inside-avoid">
      ...
      <div className="flex gap-2">
        <input type="text" className="flex-1 border border-gray-300 rounded px-3 py-2 text-sm bg-white text-black focus:ring-2 focus:ring-blue-500 outline-none" placeholder="新規項目を追加..." value={newItem} onChange={(e) => setNewItem(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleAdd()} />
        <button onClick={handleAdd} className="bg-blue-600 text-white px-4 py-2 rounded text-sm hover:bg-blue-700 whitespace-nowrap shadow-sm transition-colors"><i className="fa-solid fa-plus mr-1"></i>追加</button>
      </div>
    </div>
  );
};
    ...
    <div className="p-4 max-w-4xl mx-auto bg-gray-50 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pb-20">
        {MASTER_GROUPS[masterTab].map((key) => {
           ...
           // 新規入場者ではTraining系は不要だが、同期仕様のため表示だけはしておく
