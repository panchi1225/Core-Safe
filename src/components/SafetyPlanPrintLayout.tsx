...
const SafetyPlanPrintLayout: React.FC<Props> = ({ data }) => {
  // Styles
  const borderOuter = "border-2 border-black"; const borderThin = "border-2 border-black"; const headerBg = "bg-cyan-100";
  const borderThin = "border-2 border-black";
                 ...
                 {bottomColSpans.map((span, i) => (
                   <td key={i} colSpan={span} className={`${borderThin} align-top p-0 text-[8px]`}>
                     <div className="flex flex-col h-full">
                       {[0, 1].map(j => (
                         <div key={j} className={`flex-1 flex items-center leading-tight ${span >= 5 ? 'pl-[1em]' : 'pl-[0.5em]'}`}>                           {(data.predictions[i] || [])[j]}
                         </div>
                       ))}
                     </div>
                   </td>
              ...
              <tr className="h-[25mm]">
                 ...
                 {bottomColSpans.map((span, i) => (
                   <td key={i} colSpan={span} className={`${borderThin} align-top p-0 text-[8px]`}>
                     <div className="flex flex-col h-full">
                       {[0, 1, 2, 3, 4].map(j => (
                         <div key={j} className={`flex-1 flex items-center leading-tight ${span >= 5 ? 'pl-[1em]' : 'pl-[0.5em]'}`}>                           {(data.countermeasures[i] || [])[j]}
                         </div>
                       ))}
                     </div>
                   </td>
              ...
              <tr className="h-[15mm]">
                 ...
                 {bottomColSpans.map((span, i) => (
                   <td key={i} colSpan={span} className={`${borderThin} align-top p-0 text-[8px]`}>
                     <div className="flex flex-col h-full">
                       {[0, 1, 2].map(j => (
                         <div key={j} className={`flex-1 flex items-center leading-tight ${span >= 5 ? 'pl-[1em]' : 'pl-[0.5em]'}`}>                           {(data.inspectionItems[i] || [])[j] ? `${j + 1}. ${(data.inspectionItems[i] || [])[j]}` : ''}
                         </div>
                       ))}
                     </div>
                   </td>
