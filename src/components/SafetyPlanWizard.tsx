...
const SafetyPlanWizard: React.FC<Props> = ({ initialData, initialDraftId, onBackToMenu }) => {
  ...
  const daysInMonth = useMemo(() => { 
    ...
    for (let i = 1; i <= totalDays; i++) { 
      ...
      let colorClass = ""; 
      let bgClass = ""; 
      if (isSun || isHol) { colorClass = "text-red-600"; bgClass = "bg-red-50"; } 
                    ...
                    <div className="flex flex-col h-full">
                      {[0, 1].map(j => (
                        <div key={j} className="flex-1 border-b border-gray-300 last:border-b-0">
                          {isPreview ? (
                            <div className="w-full h-full text-[8px] leading-tight flex items-center pl-[0.5em]">{(report.predictions[i] || [])[j]}</div>
                          ) : (
                            <select className="w-full h-full bg-transparent text-[8px] outline-none appearance-none pl-[0.5em]" value={(report.predictions[i] || [])[j] || ''} onChange={(e) => { const n = report.predictions.map(a => [...(a || ["", ""])]); if (!n[i]) n[i] = ["", ""]; n[i][j] = e.target.value; updateReport({predictions: n}); }}>
                              <option value="">-</option>
                  ...
                  <td key={i} colSpan={span} className={`${borderThin} align-top p-0`}>
                    <div className="flex flex-col h-full">
                      {[0, 1, 2, 3, 4].map(j => (
                        <div key={j} className="flex-1 border-b border-gray-300 last:border-b-0">
                          {isPreview ? (
                            <div className="w-full h-full text-[8px] leading-tight flex items-center pl-[0.5em]">{(report.countermeasures[i] || [])[j]}</div>
                          ) : (
                            <select className="w-full h-full bg-transparent text-[8px] outline-none appearance-none pl-[0.5em]" value={(report.countermeasures[i] || [])[j] || ''} onChange={(e) => { const n = report.countermeasures.map(a => [...(a || ["", "", "", "", ""])]); if (!n[i]) n[i] = ["", "", "", "", ""]; n[i][j] = e.target.value; updateReport({countermeasures: n}); }}>
                              <option value="">-</option>
                  ...
                  <td key={i} colSpan={span} className={`${borderThin} align-top p-0`}>
                    <div className="flex flex-col h-full">
                      {[0, 1, 2].map(j => (
                        <div key={j} className="flex-1 border-b border-gray-300 last:border-b-0">
                          {isPreview ? (
                            <div className="w-full h-full text-[8px] leading-tight flex items-center pl-[0.5em]">{(report.inspectionItems[i] || [])[j] ? `${j + 1}. ${(report.inspectionItems[i] || [])[j]}` : ''}</div>
                          ) : (
                            <div className="flex items-center h-full">
                              <span className="text-[8px] shrink-0 pl-[0.5em]">{j + 1}.</span>
                              <select className="w-full h-full bg-transparent text-[8px] outline-none appearance-none pl-[0.3em]" value={(report.inspectionItems[i] || [])[j] || ''} onChange={(e) => { const n = report.inspectionItems.map(a => [...(a || ["", "", ""])]); if (!n[i]) n[i] = ["", "", ""]; n[i][j] = e.target.value; updateReport({inspectionItems: n}); }}>
                                <option value="">-</option>
                                {masterData.countermeasures.map(c => <option key={c} value={c}>{c}</option>)}
                              </select>
                            </div>
