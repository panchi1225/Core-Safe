...
const REMARK_TEMPLATES = [
  {
    title: "【当月協力会作業なし】",
    content: "当月は協力会社作業員の入場はなし。元請職員にて安全訓練を実施した。\n今後協力会社入場時には入場時教育および朝礼時に、教育内容の周知徹底を図る。"
  },
  {
    title: "【当月協力会作業あり/当日作業なし】",
    content: "当日は協力会社作業員は現場未入場（作業なし）。\n当月中に作業予定があるため、教育内容については各社責任者へ周知し、入場時教育および朝礼時に再教育を実施する。"
  },
  {
    title: "【当日協力会作業あり/全員参加】",
    content: "本日現場入場中の協力会社作業員全員が参加し、安全訓練を実施した。\n教育内容について理解を確認し、現場での安全管理徹底を共有した。"
  },
  {
    title: "【当日協力会作業あり/一部不参加】",
    content: "本日現場入場中の協力会社作業員のうち、一部作業員は作業都合により不参加であった。\n当該作業員については作業開始前の朝礼時に再教育を実施する。"
  }
];
...
const SafetyTrainingWizard: React.FC<Props> = ({ initialData, initialDraftId, initialStep, onBackToMenu }) => {
  ...
  // --- 定型文挿入処理 ---
  const handleRemarkSelect = (content: string) => {
    if (report.remarks && report.remarks.trim() !== "") {
      if (!window.confirm("現在入力されている内容が上書きされますが、よろしいですか？")) {
        return;
      }
    }
    updateReport('remarks', content);
  };
      ...
      <div className="bg-gray-100 p-4 rounded-lg space-y-4">
        ...
        <div className="space-y-3">
           <div className="flex items-center gap-2">
             ...
             <input
               list="training-topic-options"
               className={`flex-1 min-w-0 p-2 border rounded bg-white text-black outline-none text-sm ${getErrorClass('topic')}`}
               value={report.topic}
           ...
           <div className="flex items-center gap-2">
             ...
             <input
               list="training-topic-options"
               className={`flex-1 min-w-0 p-2 border rounded bg-white text-black outline-none text-sm ${getErrorClass('caution')}`}
               value={report.caution}
               placeholder="候補から選択、または直接入力"
               onChange={(e) => updateReport('caution', e.target.value)}
             />
           </div>
           <datalist id="training-topic-options">
             {masterData.topics.map(topic => <option key={topic} value={topic} />)}
           </datalist>
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-gray-200">
          ...
          <textarea 
            className="w-full p-3 border border-gray-300 rounded-lg bg-white text-sm focus:ring-2 focus:ring-blue-200 outline-none mb-3"
            rows={3}
          ...
          <div className="space-y-2">
            <p className="text-xs text-gray-500 font-bold">▼ 定型文を挿入 (クリックで反映)</p>
            {REMARK_TEMPLATES.map((template, idx) => (
              <button 
                key={idx}
                onClick={() => handleRemarkSelect(template.content)}
                className="w-full text-left p-2 bg-white border border-gray-300 rounded hover:bg-blue-50 text-xs text-gray-700 transition-colors flex items-center gap-2 group"
              >
                <i className="fa-solid fa-pen-to-square text-gray-400 group-hover:text-blue-500"></i>
  ...
  return (
    <>
      <div className="no-print min-h-screen pb-24 bg-gray-50 overflow-x-hidden">
        ...
        <footer className="fixed bottom-0 left-0 w-full bg-white border-t p-4 flex justify-between items-center shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] z-20">
          <div className="flex items-center gap-2"><button onClick={handleBack} disabled={step === 1} className={`px-4 py-3 rounded-lg font-bold ${step === 1 ? 'text-gray-300' : 'text-gray-600 bg-gray-100'}`}>戻る</button><button onClick={handleSave} className="px-4 py-3 rounded-lg font-bold border border-blue-200 text-blue-600 bg-blue-50 hover:bg-blue-100 flex items-center"><i className={`fa-solid ${saveStatus === 'saved' ? 'fa-check' : 'fa-save'} mr-2`}></i>{saveStatus === 'saved' ? '保存完了' : '保存'}</button></div>
